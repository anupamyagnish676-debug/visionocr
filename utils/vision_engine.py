...
def _get_model(genai, key: str):
    """
    Try models in order until one works.
    Returns the first working GenerativeModel.
    """
    ...
    try:
        ...
        for c in candidates:
            if c in available:
                print(f'[Gemini] Using model: {c}')
                return genai.GenerativeModel(c)

        # Fallback: use first available vision-capable model
        vision_models = [m for m in available if 'flash' in m or 'pro' in m or 'vision' in m]
        if vision_models:
            print(f'[Gemini] Fallback model: {vision_models[0]}')
...
def analyze_image(
    ...
    if not key:
        raise ValueError(
            ...
            "Paste your key in app/__init__.py line 6."
        )

    model = _get_model(genai, key)
...
"""
    ...
    prompt = f"""You are an expert OCR and document analysis assistant.
